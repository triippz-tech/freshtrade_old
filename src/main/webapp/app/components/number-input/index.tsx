...
const isEqual = (oldValue, newValue) =>
  newValue === oldValue || (typeof newValue === 'number' && typeof oldValue === 'number' && isNaN(newValue) && isNaN(oldValue));

class NumberInput extends React.Component<Partial<NumberInputProps>, NumberInputState> {
  static defaultProps = {
    focusOnUpDown: true,
    useTouch: false,
  ...
  upStep(val, rat) {
    ...
    const result = ((precisionFactor * val + precisionFactor * (step as number) * rat) / precisionFactor).toFixed(precision);
    ...
    return (
      ...
      >
        <div className={`${prefixCls}-handler-wrap`}>
          ...
          >
            {upHandler || <span unselectable="on" className={`${prefixCls}-handler-up-inner`} onClick={preventDefault} />}
          </span>
          <span
            unselectable="on"
            {...(downEvents as any)}
          ...
          >
            {downHandler || <span unselectable="on" className={`${prefixCls}-handler-down-inner`} onClick={preventDefault} />}
          </span>
        </div>
