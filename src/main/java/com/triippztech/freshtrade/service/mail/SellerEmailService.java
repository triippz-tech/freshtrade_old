...
public class SellerEmailService {
    ...
    private final Logger log = LoggerFactory.getLogger(SellerEmailService.class);
    ...
    @Async
    public void sendSellerConfirmReservationEmail(Reservation reservation, User buyer, Item foundItem, Integer quantity)
        throws IOException {
        log.debug("Sending Seller Reservation Confirmation Email To: {}", buyer.getEmail());
        ...
        context.setVariable("buyer", buyer.getLogin());
        ...
        String content = templateEngine.process("mail/sellerReserveConfirmEmail", context);
        String subject = messageSource.getMessage("email.seller.reserve.confirm.title", null, locale);
